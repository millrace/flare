...
| `flare.tls` | TLS 1.2+ only, weak ciphers disabled, SNI always sent. Client and server both supported. The server handshake runs non-blocking on the reactor (`TlsConnHandle`), so a slow or stalled handshake occupies a connection slot bounded by `ServerConfig.idle_timeout_ms` rather than a worker. The blocking `TlsAcceptor.handshake_fd(fd)` path remains for callers driving TLS outside the reactor. |
...
| `flare.http2` (server) | RFC 9113 §6.5.2 `SETTINGS_MAX_HEADER_LIST_SIZE = 16 KiB` advertised **and enforced** on inbound HEADERS + CONTINUATION accumulation: an oversized header block is closed with `RST_STREAM(ENHANCE_YOUR_CALM)` (a 1 MiB hard cap applies even when the setting is unset). A CONTINUATION flood trips `GOAWAY(ENHANCE_YOUR_CALM)` (CVE-2024-27316) -- connection-level because an unterminated header block leaves the shared HPACK context unresynchronizable -- and an inbound `RST_STREAM` flood trips the same (CVE-2023-44487 rapid reset). `SETTINGS_MAX_CONCURRENT_STREAMS = 100` advertised **and enforced**: a stream past the cap is answered `RST_STREAM(REFUSED_STREAM)`. Frame shape, stream state, pseudo-header well-formedness and flow-control limits are validated per RFC 9113 §4-§8 and reported with `GOAWAY` carrying the §7 error code; `pixi run conformance` runs h2spec against a live server in CI. |
